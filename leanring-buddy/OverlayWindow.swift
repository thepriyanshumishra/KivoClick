...
struct BlueCursorView: View {
    ...
    private var activePointerColor: Color {
        switch selectedCursorColorId {
    ...
    private let fullWelcomeMessage = "hey! i'm kivo click"
    ...
    var body: some View {
        ZStack {
                ...
                Text(welcomeText)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            .fill(activePointerColor)
                            .shadow(color: activePointerColor.opacity(0.5), radius: 6, x: 0, y: 0)
                    )
                    .fixedSize()
                    .overlay(
                ...
                Text(navigationBubbleText)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            .fill(activePointerColor)
                            .shadow(
                                color: activePointerColor.opacity(0.5 + (1.0 - navigationBubbleScale) * 1.0),
            ...
            Triangle()
                ...
                .shadow(color: activePointerColor, radius: 8 + (buddyFlightScale - 1.0) * 20, x: 0, y: 0)
            ...
            BlueCursorWaveformView(audioPowerLevel: companionManager.currentAudioPowerLevel, color: activePointerColor)
            ...
            BlueCursorSpinnerView(color: activePointerColor)
        ...
        .onChange(of: companionManager.currentPointingTarget) { _, newTarget in
            guard let target = newTarget else {
                // Return to cursor if we are currently pointing or navigating
                if buddyNavigationMode != .followingCursor && !isReturningToCursor {
            ...
            guard screenFrame.contains(CGPoint(x: target.displayFrame.midX, y: target.displayFrame.midY))
                  || target.displayFrame == screenFrame else {
                return
            }

            startNavigatingToElement(target: target)
        }
    }
    ...
    private func startNavigatingToElement(target: PointingTarget) {
        // Don't interrupt welcome animation
        guard !showWelcome || welcomeText.isEmpty else { return }

        // Convert the AppKit screen location to SwiftUI coordinates for this screen
        let targetInSwiftUI = convertScreenPointToSwiftUICoordinates(target.screenLocation)
        ...
        animateBezierFlightArc(to: clampedTarget) {
            ...
            self.startPointingAtElement(target: target)
        }
    }
    ...
    private func startPointingAtElement(target: PointingTarget) {
        ...
        let pointerPhrase = target.label

        streamNavigationBubbleCharacter(phrase: pointerPhrase, characterIndex: 0) {
            // All characters streamed — wait for the manager to clear us or transition to another target
        }
    }
